...
describe('2020/12/18', () => {
  test('explode', () => {
    expect(new SnailfishNumber([[[[[9, 8], 1], 2], 3], 4]).step().flatten())
      .toEqual([[[[0, 9], 2], 3], 4]);
    expect(new SnailfishNumber([7, [6, [5, [4, [3, 2]]]]]).step().flatten())
      .toEqual([7, [6, [5, [7, 0]]]]);
    expect(new SnailfishNumber([[6, [5, [4, [3, 2]]]], 1]).step().flatten())
      .toEqual([[6, [5, [7, 0]]], 3]);
    expect(new SnailfishNumber([[3, [2, [1, [7, 3]]]], [6, [5, [4, [3, 2]]]]]).step().flatten())
      .toEqual([[3, [2, [8, 0]]], [9, [5, [4, [3, 2]]]]]);
    expect(new SnailfishNumber([[3, [2, [8, 0]]], [9, [5, [4, [3, 2]]]]]).step().flatten())
      .toEqual([[3, [2, [8, 0]]], [9, [5, [7, 0]]]]);
  });

  test('split', () => {
    expect(new SnailfishNumber([[[[0, 7], 4], [15, [0, 13]]], [1, 1]]).step().flatten())
      .toEqual([[[[0, 7], 4], [[7, 8], [0, 13]]], [1, 1]]);
    expect(new SnailfishNumber([[[[0, 7], 4], [[7, 8], [0, 13]]], [1, 1]]).step().flatten())
      .toEqual([[[[0, 7], 4], [[7, 8], [0, [6, 7]]]], [1, 1]]);
  });

  test('run', () => {
    expect(new SnailfishNumber([[[[[4, 3], 4], 4], [7, [[8, 4], 9]]], [1, 1]]).run())
